...
from .ctx import CTX
from ..universe.shipnames import SHIPNAMES, ID_BY_SHIPNAMES
from typing import Tuple

# todo pars "  " as seperator generated by log engine to sep drop text elements forst
def textReplace(element, new_text, sfx = ""):
    """
        replace the text
    Args:
...
def removeSuffix(text:str):
    upper_text = text.upper()
    for sfx in CTX.CHARS_TO_IGNORE:
        text = text.replace(sfx, "  " )

    for sfx in CTX.STATUS_CLEAR:
        if upper_text.endswith(sfx ):
            return text[ 0: -len(sfx)].strip(), sfx

    for sfx in CTX.STATUS_BLUE:
        if upper_text.endswith(sfx ):
            return text[ 0: -len(sfx)].strip(), sfx

    for sfx in CTX.STATUS_STATUS:
        if upper_text.endswith(sfx ):
            return text[ 0: -len(sfx)].strip(), sfx

    for sfx in CTX.WORDS_TO_IGNORE:
        if upper_text.endswith(sfx ):
            return text[ 0: -len(sfx)].strip(), text[ -len(sfx):]
    return text, ""

def parsePlayerNames(rtext)->bool:
    """
    Parse all NavigableString's and replace player names with a link to zKillboard
    Player names dropped by users hold two spaces as postfix
    see:
    ...
    Returns:
        True : at least one player was found

    """
    token_replaced = False
    for text in [t for t in rtext.contents if isinstance(t, NavigableString)]:
        tokens, sfx = removeSuffix(text.strip())
        tokens = tokens.split("  ")
        new_xml_tags = ""
        while tokens:
            search_text = tokens[0]
            name_sep = search_text.split(" ") #  - '
            # check A-Z, a-z, die Nummern 0-9, sowie Gedankenstriche und Apostrophen
            length_valid = (len(search_text) >= 3) and (len(search_text) <= 37)
            first_name_valid =  (len(search_text) >= 3) and (len(name_sep) >0) and (len(name_sep) <=3) and (len(name_sep[0]) <= 24 )
            last_name_valid = (length_valid and  (len(name_sep) >0 ))or(len(search_text) >= 3) and(len(name_sep) >1) and (len(name_sep) <=3) and (len(name_sep[-1]) <= 12)
            if length_valid and first_name_valid and last_name_valid:
                res, player_id = checkPlayerName(search_text)
                if res == EXISTS:
                    new_xml_tags = new_xml_tags + CTX.FORMAT_PLAYER_NAME.format(search_text, player_id)
                    token_replaced = True
                else:
                    new_xml_tags = new_xml_tags + " " + search_text + sfx
                    sfx = ""
            else:
                new_xml_tags = new_xml_tags + " " + search_text + sfx
                sfx = ""
            tokens = tokens[1:]
        if token_replaced:
            textReplace(element=text, new_text=new_xml_tags, sfx=sfx)
...
def parseShips(rtext) -> bool:
    ...
    """
    def formatShipName(in_text, in_name,type_name):
        return in_text.replace(in_name, CTX.FORMAT_SHIP.format(in_name, type_name))

    texts = [t for t in rtext.contents if isinstance(t, NavigableString)]
    for text in texts:
        upper_text = text.upper()
        for shipName in [shipName for shipName in ID_BY_SHIPNAMES.keys() if shipName in upper_text]:
            ...
            formatted = formatShipName(text, ship_in_text, SHIPNAMES[ID_BY_SHIPNAMES[shipName]])
...
LAST_USER_MESSAGE:dict[str:Tuple[int,str]] = dict()
...
def parseMessageForMap(systems_on_map: dict[str, System], message: Message) -> Message:
    ...
    if CTX.ZKILLBOARD_ROOM_NAME != message.roomName:
    ...
    if not message.affectedSystems and message.user:
        systems = LAST_USER_MESSAGE.get(message.user)
        if systems:
            message.affectedSystems = systems[1]
